...
describe('deferred Umami loader', () => {
  it('queues dashboard analytics calls and flushes them after the deferred script loads', async () => {
    ...
    try {
      ...
      assert.equal(firstScript.dataset.excludeSearch, 'true', 'deferred URL credentials must not enter Umami pageviews');
      assert.equal(firstScript.dataset.excludeHash, 'true', 'fragment credentials must not enter Umami pageviews');
